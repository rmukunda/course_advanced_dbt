...
{% set dim_subscriptions = unit_testing_table_select(ref('dim_subscriptions'), ref('unit_test_subscriptions')) %}
{% set dim_dates = unit_testing_table_select(ref('dim_dates'), ref('unit_test_dates')) %}
...
monthly_subscriptions AS (
    SELECT
        ...
        plan_name,
        pricing,
        {{ date_month('starts_at') }} AS start_month,
        {{ date_month('ends_at') }} AS end_month
    FROM
        {{ dim_subscriptions }}
    WHERE
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATE(DATEADD('month', 1, {{ date_month('current_date') }}))
            ELSE end_month
        END AS end_month
    FROM
...
final AS (
    ...
    WHERE
        date_month <= {{ date_month('current_date') }}
)
